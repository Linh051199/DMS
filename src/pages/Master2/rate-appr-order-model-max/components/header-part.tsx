...
export const HeaderPart = ({ onAddNew, searchCondition }: IHeaderPartProps) => {
  ...
  const handleUploadFiles = async (files: File[]) => {
    const resp = await api.Mst_RateApprOrderModelMax_Import(files[0]);
    if (resp.isSuccess) {
  ...
  const handleExportExcel = async (selectedOnly: boolean) => {
    ...
    let resp = await match(selectedOnly)
      .with(true, async () => {
        return await api.Mst_RateApprOrderModelMax_ExportByListDealerCode(selectedItems);
      })
      .otherwise(async () => {
        return await api.Mst_RateApprOrderModelMax_Export(searchCondition);
      });
    if (resp.isSuccess) {
      toast.success(t("DownloadSuccessfully"));
      window.location.href = resp.Data;
  ...
  return (
    <PageHeaderNoSearchLayout>
      <PageHeaderNoSearchLayout.Slot name={"Before"}>
        <div className="font-bold dx-font-m">Thiết lập tỉ lệ duyệt đơn hàng tối đa theo Model</div>
