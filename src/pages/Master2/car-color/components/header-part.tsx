...
import { SearchCarColor } from "@/packages/types";
...
import { selectedItemsAtom } from "./car-color-store";

interface IHeaderPartProps {
  onAddNew: () => void;
  searchCondition: Partial<SearchCarColor>;
}
...
export const HeaderPart = ({ onAddNew, searchCondition }: IHeaderPartProps) => {
  ...
  const handleUploadFiles = async (files: File[]) => {
    const resp = await api.Mst_CarColor_Import(files[0]);
    if (resp.isSuccess) {
  ...
  const onDownloadTemplate = async () => {
    const resp = await api.Mst_CarColor_Export_Template();
    if (resp.isSuccess) {
  ...
  const handleExportExcel = async (selectedOnly: boolean) => {
    ...
    let resp = await match(selectedOnly)
      .with(true, async () => {
        return await api.Mst_CarColor_ExportByListCarColorCode(selectedItems);
      })
      .otherwise(async () => {
        return await api.Mst_CarColor_Export(searchCondition);
      });
    if (resp.isSuccess) {
      toast.success(t("DownloadSuccessfully"));
      window.location.href = resp.Data;
  ...
  return (
    <PageHeaderNoSearchLayout>
      <PageHeaderNoSearchLayout.Slot name={"Before"}>
        <div className="font-bold dx-font-m">Quản lý màu sắc</div>
