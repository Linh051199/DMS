...
import { Search_Mst_Bank } from "@/packages/types";
...
import { selectedItemsAtom } from "./bank-store";

interface IHeaderPartProps {
  onAddNew: () => void;
  searchCondition: Partial<Search_Mst_Bank>;
}
...
export const HeaderPart = ({ onAddNew, searchCondition }: IHeaderPartProps) => {
  ...
  const handleUploadFiles = async (files: File[]) => {
    const resp = await api.Mst_Bank_ImportExcel(files[0]);
    if (resp.isSuccess) {
  ...
  const onDownloadTemplate = async () => {
    const resp = await api.Mst_Bank_ExportTemplate();
    if (resp.isSuccess) {
  ...
  const handleExportExcel = async (selectedOnly: boolean) => {
    ...
    let resp = await match(selectedOnly)
      .with(true, async () => {
        return await api.Mst_Bank_ExportByListBankCodeType(selectedItems);
      })
      .otherwise(async () => {
        return await api.Mst_Bank_ExportExcel();
      });
    if (resp.isSuccess) {
      toast.success(t("DownloadSuccessfully"));
      window.location.href = resp.Data;
  ...
  return (
    <PageHeaderNoSearchLayout>
      <PageHeaderNoSearchLayout.Slot name={"Before"}>
        <div className="font-bold dx-font-m">Quản lý ngân hàng</div>
