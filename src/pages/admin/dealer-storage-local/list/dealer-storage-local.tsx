...
import "./dealer-storage-local.scss";
...
export const DealerStorageGlobalPage = () => {
  ...
  const columns: ColumnOptions[] = useMemo(
    () => [
...

      {
        ...
        headerFilter: {
          alowwSearch: true,
          dataSource: uniqueFilterByDataField(data?.DataList, "DealerName"),
        },
      },
  ...
  const handleEditorPreparing = (e: EditorPreparingEvent<any, any>) => {
    ...
    } else if (e.dataField === "StorageCode") {
      e.editorOptions.readOnly = !e.row?.isNewRow;
  ...
  const handleDelete = async (key: string) => {
    const resp = await api.Dlr_StorageLocal_Delete(key);
    if (resp.isSuccess) {
  ...
  const handleCreate = async (data: any) => {
    const res = await api.Dlr_StorageLocal_Create({ ...data });
    if (res.isSuccess) {
  ...
  const handleUpdate = async (key: string, data: any) => {
    const resp = await api.Dlr_StorageLocal_Update(key, data);
    if (resp.isSuccess) {
  ...
  return (
    <AdminContentLayout className={"dealer-storage-local"}>
      <AdminContentLayout.Slot name="Header">
        <PageHeaderLayout>
          <PageHeaderLayout.Slot name="Before">
            <div className="font-bold dx-font-m">
              Thiết lập kho khu vực cho đại lý
            </div>
      ...
      <AdminContentLayout.Slot name="Content">
        <BaseGridView
          ...
          storeKey={"dealer-storage-global-columns"}
