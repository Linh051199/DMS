...
export const PlantPage = () => {
  ...
  const columns: ColumnOptions[] = useMemo(
    () => [
      ...
      },
      {
        ...
        headerFilter: {
          alowwSearch: true,
          dataSource: uniqueFilterByDataField(data?.DataList, "PlantName"),
        },
        validationRule: [requiredType, ExcludeSpecialCharactersType],
      },
      {
  ...
  const handleDelete = async (key: string) => {
    const resp = await api.Mst_Plant_Delete(key);
    if (resp.isSuccess) {
  ...
  const handleCreate = async (data: any) => {
    const res = await api.Mst_Plant_Create({ ...data });
    if (res.isSuccess) {
  ...
  const handleUpdate = async (key: string, data: any) => {
    const resp = await api.Mst_Plant_Update(key, data);
    if (resp.isSuccess) {
  ...
  const handleDeleteRows = async (rows: string[]) => {
    const resp = await api.Mst_Plant_DeleteMultiple(rows);
    if (resp.isSuccess) {
  ...
  return (
    <AdminContentLayout className={"plant"}>
      <AdminContentLayout.Slot name="Header">
        <PageHeaderLayout>
          <PageHeaderLayout.Slot name="Before">
            <div className="font-bold dx-font-m">Quản lý nhà máy</div>
      ...
      <AdminContentLayout.Slot name="Content">
        <BaseGridView
          keyExpr="PlantCode"
          storeKey={"plant-management-columns"}
