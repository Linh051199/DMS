...
import { selectedItemsAtom } from "./car-spec-store";
...
export const HeaderPart = ({ onAddNew }: IHeaderPartProps) => {
  const { t } = useI18n("CarSpec");
  const selectedItems = useAtomValue(selectedItemsAtom);
  const api = useClientgateApi();
  const showError = useSetAtom(showErrorAtom);

  const handleUploadFiles = async (files:File[] ) => {
    const resp = await api.Mst_CarSpec_Import(files[0]);
    if (resp.isSuccess) {
  ...
  const handleExportExcel = async (selectedOnly: boolean) => {
    ...
    let resp = await match(selectedOnly)
      .with(true, async () => {
        return await api.Mst_CarSpec_ExportByListCode(selectedItems);
      })
      .otherwise(async () => {
  ...
  return (
    <PageHeaderNoSearchLayout>
      <PageHeaderNoSearchLayout.Slot name={"Before"}>
        <div className="font-bold dx-font-m">Quản lý đặc tả xe</div>
