...
import "./auto-mapVIN-storage-rate.scss";
...
export const AutoMapVINStorageRatePage = () => {
  ...
  const { data: listStorage } = useQuery(["listStorage"], () => {
    return api.Mst_Storage_GetAllActive();
  });
  ...
  const { data: listColor } = useQuery(["listColor"], () => {
    return api.Mst_CarColor_GetAllActive();
  });
  useEffect(() => {
  ...
  const findColorExtNameVN = (key: any) => {
    const findColorExtNameVN = listColor?.DataList?.find(
      (i) => i.ColorExtCode === key
    );

    return findColorExtNameVN?.ColorExtNameVN;
  };
  ...
  const columns: ColumnOptions[] = useMemo(
    () => [
...

      {
        ...
        validationRule: [requiredType, ExcludeSpecialCharactersType],
        setCellValue: (
          newData: Partial<Auto_MapVIN_StorageRate>,
          value: string
        ) => {
...

      {
        ...
        setCellValue: (
          ...
          newData.ColorExtNameVN = findColorExtNameVN(value);
        },
      },

      {
        ...
        headerFilter: {
          alowwSearch: true,
          dataSource: uniqueFilterByDataField(data?.DataList, "ColorExtNameVN"),
        },
        validationRule: [requiredType, ExcludeSpecialCharactersType],
      },

      {
        caption: "Tỉ lệ map miền Bắc (%)",
        dataField: "MBVal",
        editorType: "dxTextBox",
        visible: true,
        editorOptions: {
        ...
        headerFilter: {
          alowwSearch: true,
          dataSource: uniqueFilterByDataField(data?.DataList, "MBVal"),
        },
        validationRule: [requiredType, ExcludeSpecialCharactersType],
      },

      {
        caption: "Tỉ lệ map miền Trung (%)",
        dataField: "MTVal",
        editorType: "dxTextBox",
        visible: true,
        editorOptions: {
        ...
        headerFilter: {
          alowwSearch: true,
          dataSource: uniqueFilterByDataField(data?.DataList, "MTVal"),
        },
        validationRule: [requiredType, ExcludeSpecialCharactersType],
      },

      {
        caption: "Tỉ lệ map miền Nam (%)",
        dataField: "MNVal",
        editorType: "dxTextBox",
        visible: true,
        editorOptions: {
        ...
        headerFilter: {
          alowwSearch: true,
          dataSource: uniqueFilterByDataField(data?.DataList, "MNVal"),
        },
        validationRule: [requiredType, ExcludeSpecialCharactersType],
      },
    ],
    [data, listModel, listStorage, listColor]
  );
  ...
  const handleEditorPreparing = (e: EditorPreparingEvent<any, any>) => {
    if (e.dataField) {
      ...
      if (e.row?.isEditing === true && !e.row.isNewRow) {
        if (
          ["StorageCode", "ModelCode", "SpecCode", "ColorExtCode"].includes(
            e.dataField
          )
        ) {
          e.editorOptions.readOnly = true;
  ...
  const handleDelete = async (key: string) => {
    const resp = await api.Auto_MapVIN_StorageRate_Delete(key);
    if (resp.isSuccess) {
  ...
  const handleCreate = async (data: any) => {
    const res = await api.Auto_MapVIN_StorageRate_Create({ ...data });
    if (res.isSuccess) {
  ...
  const handleUpdate = async (key: string, data: any) => {
    const resp = await api.Auto_MapVIN_StorageRate_Update(key, data);
    if (resp.isSuccess) {
  ...
  const handleDeleteRows = async (rows: string[]) => {
    const resp = await api.Auto_MapVIN_StorageRate_DeleteMultiple(rows);
    if (resp.isSuccess) {
  ...
  return (
    <AdminContentLayout className={"auto-mapVIN-storage-rate"}>
      <AdminContentLayout.Slot name="Header">
        <PageHeaderLayout>
          <PageHeaderLayout.Slot name="Before">
            <div className="font-bold dx-font-m">
              Thiết lập phân bổ map vin theo kho toàn quốc
            </div>
      ...
      <AdminContentLayout.Slot name="Content">
        <BaseGridView
          ...
          storeKey={"Auto_MapVIN_StorageRate_Columns"}
