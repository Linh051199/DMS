...
export const UnitPriceAVNPage = () => {
    ...
    () => [
      {
        ...
        headerFilter: {
          alowwSearch: true,
          dataSource: uniqueFilterByDataField(data?.DataList, "AVNCode"),
        },
...

      {
        ...
        headerFilter: {
          alowwSearch: true,
          dataSource: uniqueFilterByDataField(data?.DataList, "UnitPriceAVN"),
        },
        validationRule: [requiredType, ExcludeSpecialCharactersType],
      },

      {
        ...
        headerFilter: {
          alowwSearch: true,
          dataSource: uniqueFilterByDataField(data?.DataList, "EffDateTime"),
        },
        validationRule: [requiredType, ExcludeSpecialCharactersType],
      },
  ...
  const handleDelete = async (key: Partial<Mst_UnitPriceAVN>) => {
    const resp = await api.Mst_UnitPriceAVN_Delete(key);
    if (resp.isSuccess) {
  ...
  const handleCreate = async (data: any) => {
    const res = await api.Mst_UnitPriceAVN_Create({ ...data });
    if (res.isSuccess) {
  ...
  const handleUpdate = async (key: string, data: any) => {
    const resp = await api.Mst_UnitPriceAVN_Update(key, data);
    if (resp.isSuccess) {
  ...
  const handleDeleteRows = async (rows: string[]) => {
    const resp = await api.Mst_UnitPriceAVN_DeleteMultiple(rows);
    if (resp.isSuccess) {
  ...
  return (
    <AdminContentLayout className={"unit-price-avn"}>
      <AdminContentLayout.Slot name="Header">
        <PageHeaderLayout>
          <PageHeaderLayout.Slot name="Before">
            <div className="font-bold dx-font-m">Quản lý bảng giá AVN</div>
      ...
      <AdminContentLayout.Slot name="Content">
        <BaseGridView
          ...
          storeKey={"Mst_UnitPriceAVN_Columns"}
