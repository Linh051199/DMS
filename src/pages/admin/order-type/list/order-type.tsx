...
export const OrderTypePage = () => {
  ...
  useEffect(() => {
    if (!!orderTypeData && !orderTypeData.isSuccess) {
      showError({
        message: t(orderTypeData.errorCode),
        debugInfo: orderTypeData.debugInfo,
        errorInfo: orderTypeData.errorInfo,
      });
    }
  }, [orderTypeData]);
  ...
  const columns: ColumnOptions[] = useMemo(
    () => [
      ...
      },
      {
        ...
        headerFilter: {
          dataSource: filterByFlagActive(orderTypeData?.DataList, {
            true: t("Active"),
            false: t("Inactive"),
          }),
        },
  ...
  const handleDelete = async (key: string) => {
    const resp = await api.Mst_SalesOrderType_Delete(key);
    if (resp.isSuccess) {
  ...
  const handleCreate = async (data: any) => {
    const res = await api.Mst_SalesOrderType_Create({ ...data });
    if (res.isSuccess) {
  ...
  const handleUpdate = async (key: string, data: any) => {
    const resp = await api.Mst_SalesOrderType_Update(key, data);
    if (resp.isSuccess) {
  ...
  const handleDeleteRows = async (rows: string[]) => {
    const resp = await api.Mst_SalesOrderType_DeleteMultiple(rows);
    if (resp.isSuccess) {
  ...
  return (
    <AdminContentLayout className={"order-type"}>
      <AdminContentLayout.Slot name="Header">
        <PageHeaderLayout>
          <PageHeaderLayout.Slot name="Before">
            <div className="font-bold dx-font-m">Quản lý loại đơn hàng</div>
      ...
      <AdminContentLayout.Slot name="Content">
        <BaseGridView
          ...
          isLoading={isLoading}
          dataSource={orderTypeData?.DataList ?? []}
          columns={columns}
          allowSelection={true}
