...
import "./transporter-driver.scss";
...
export const TransporterDiverPage = () => {
  ...
  const columns: ColumnOptions[] = useMemo(
    () => [
      ...
      },
      {
        ...
        headerFilter: {
          dataSource: uniqueFilterByDataField(data?.DataList, "DriverId"),
          allowSearch: true,
        },
        validationRule: [requiredType, ExcludeSpecialCharactersType],
      },

      {
        ...
        headerFilter: {
          dataSource: uniqueFilterByDataField(data?.DataList, "DriverFullName"),
          allowSearch: true,
        },
        validationRule: [requiredType, ExcludeSpecialCharactersType],
      },
      ...
      {
        caption: "Số hộ chiếu/ bằng lái xe/ CMND",
...

      {
        ...
        headerFilter: {
          dataSource: uniqueFilterByDataField(data?.DataList, "DriverPhoneNo"),
          allowSearch: true,
        },
        validationRule: [requiredType, ExcludeSpecialCharactersType],
      },

      {
        ...
        alignment: "center",
        cellRender: ({ data }: any) => (
          <StatusButton isActive={data.FlagActive} />
        ),
        headerFilter: {
  ...
  const handleDelete = async (key: string) => {
    const resp = await api.Mst_TransporterDriver_Delete(key);
    if (resp.isSuccess) {
  ...
  const handleCreate = async (data: any) => {
    const res = await api.Mst_TransporterDriver_Create({ ...data });
    if (res.isSuccess) {
  ...
  const handleUpdate = async (key: string, data: any) => {
    const resp = await api.Mst_TransporterDriver_Update(key, data);
    if (resp.isSuccess) {
  ...
  return (
    <AdminContentLayout className={"transporter-driver"}>
      ...
      <AdminContentLayout.Slot name="Content">
        <BaseGridView
          ...
          storeKey={"transport-driver-columns"}
