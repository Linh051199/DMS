...
import { RptUpdSalesManByDelearParam } from "@/packages/api/clientgate/Rpt_UpdSalesManByDelearApi";
...
interface RptBaoCaoMoiParam {
  // Date_From: string;
  // Date_To: string;
  FlagDataWH: 1 | 0;
}

export const Rpt_UpdSalesManByDelear = () => {
  const { t } = useI18n("Rpt_UpdSalesManByDelear");
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      if (loadingKey !== "0") {
        const resp = await api.RptUpdSalesManByDelear_SearchHQ({
          FlagDataWH: searchCondition.FlagDataWH ? 1 : 0,
        } as RptUpdSalesManByDelearParam);
        return resp;
      } else {
        return null;
      }
  ...
  const dataSource = new PivotGridDataSource({
    fields: fields,
    store: data?.Data?.Lst_Rpt_UpdSalesManByDealer,
  });
        ...
        <ContentSearchPanelLayout>
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <div className="w-[200px]">
              <SearchPanelV2
                ...
                storeKey={"Rpt_UpdSalesManByDelear-search"}
              />
            </div>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            ...
            <div className="w-full mt-4">
              {!!data && data?.Data?.Lst_Rpt_UpdSalesManByDealer && (
                <PivotGrid
                  id="pivotgrid"
                ...
                >
                  ...
                  <StateStoring
                    enabled={true}
                    storageKey={"Rpt_UpdSalesManByDelear"}
                  />
