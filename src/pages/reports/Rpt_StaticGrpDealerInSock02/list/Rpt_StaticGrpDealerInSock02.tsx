...
import { RptStatisticGrpDealerInStock02Param } from "@/packages/api/clientgate/Rpt_StatisticGrpDealerInStock02Api";
...
export const RptStaticGrpDealerInSock02 = () => {
  const { t } = useI18n("RptStaticGrpDealerInSock02");
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      if (loadingKey !== "0") {
        const resp = await api.RptStaticGrpDealerInSock02_SearchHQ({
          FlagDataWH: searchCondition.FlagDataWH ? 1 : 0,
        } as RptStatisticGrpDealerInStock02Param);
        return resp;
      } else {
        return null;
      }
  ...
  const handleExportExcelDetail = useCallback(async () => {
    const result = await api.RptStaticGrpDealerInSock02_ExportDetailSearchHQ({
      FlagDataWH: searchCondition.FlagDataWH ? 1 : 0,
    });
  ...
  const dataSource = new PivotGridDataSource({
    fields: fields,
    store: data?.Data?.Lst_RptStatistic_GrpDealerInStock02,
  });
        ...
        <ContentSearchPanelLayout>
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <div className="w-[200px]">
              <SearchPanelV2
                ...
                storeKey={"RptStaticGrpDealerInSock02-search"}
              />
            </div>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            ...
            <div className="w-full mt-4">
              {!!data && data?.Data?.Lst_RptStatistic_GrpDealerInStock02 && (
                <PivotGrid
                  id="pivotgrid"
                ...
                >
                  ...
                  <StateStoring
                    enabled={true}
                    storageKey={"RptStaticGrpDealerInSock02"}
                  />
