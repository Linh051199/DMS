...
export const Rpt_BaoCaoMoi = () => {
  const { t } = useI18n("Rpt_BaoCaoMoi");
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      if (loadingKey !== "0") {
        const resp = await api.RptBaoCaoMoi_SearchHQ({
          FlagDataWH: searchCondition.FlagDataWH ? 1 : 0,
        } as RptBaoCaoMoiParam);
        return resp;
      } else {
        return null;
      }
  ...
  const handleExportExcelDetail = useCallback(async () => {
    const result = await api.RptBaoCaoMoi_ExportDetailSearchHQ({
      FlagDataWH: searchCondition.FlagDataWH ? 1 : 0,
    });
  ...
  const fields = useMemo<Field[]>(() => {
    return [
      ...
      {
        dataField: "TrangThaiBackOrderText",
        area: "column",
        areaIndex: 0,
      },
    ];
  ...
  const dataSource = new PivotGridDataSource({
    fields: fields,
    store: data?.Data?.Lst_Rpt_BaoCaoMoi,
  });
        ...
        <ContentSearchPanelLayout>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            ...
            <div className="w-full mt-4">
              {!!data && data?.Data?.Lst_Rpt_BaoCaoMoi && (
                <PivotGrid
                  id="pivotgrid"
                ...
                >
                  ...
                  <StateStoring enabled={true} storageKey={"Rpt_BaoCaoMoiZ"} />
