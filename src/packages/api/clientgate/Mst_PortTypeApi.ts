import { ApiResponse, Mst_PortType, SearchParam } from "@packages/types";
...
export const useMstPortTypeAPI = (apiBase: AxiosInstance) => {
  return {
    ...
    Mst_PortType_ExportExcel: async (
      ...
      {
        return await apiBase.post<Partial<Mst_PortType>, ApiResponse<string>>(
          "/MstPortType/Export",
          {
            KeyWord: keyword,
            FlagActive: "",
    ...
    Mst_PortType_ExportByListPortType: async (
      keys: string[]
    ): Promise<ApiResponse<any>> => {
      return await apiBase.post<Partial<Mst_PortType>, ApiResponse<string>>(
        "/MstPortType/ExportByListPortType",
        {
          ListPortType: keys.join(","),
        }
      );
    },
