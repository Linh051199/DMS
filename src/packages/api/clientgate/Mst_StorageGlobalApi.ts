...
export const useMst_StorageGlobalApi = (apiBase: AxiosInstance) => {
  return {
    ...
    Mst_StorageGlobal_GetByStorageCodeAndModelCode: async (
      StorageCode: string
    ): Promise<ApiResponse<Mst_StorageGlobal>> => {
      return await apiBase.post<string, ApiResponse<Mst_StorageGlobal>>(
        "/MstStorageGlobal/GetByStorageCodeAndModelCode",
        {
    ...
    Mst_StorageGlobal_ExportByListStorageCodeAndModelCode: async (
      object: any
    ): Promise<ApiResponse<any>> => {
      {
        return await apiBase.post<
          Partial<Mst_StorageGlobal>,
          ApiResponse<string>
        >("/MstStorageGlobal/ExportByListStorageCodeAndModelCode", {
          ListStorageCodeAndModelCode: object,
        });
      }
    },
  };
